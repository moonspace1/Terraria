...
public class PlayerInputProfile
{
	...
	public bool Load(Dictionary<string, object> dict)
	{
		...
		{
			InputModes[InputMode.Keyboard].ReadPreferences(JsonConvert.DeserializeObject<Dictionary<string, List<string>>>(((JObject)value).ToString()));
		}
		if (dict.TryGetValue("Gamepad", out value))
		{
			InputModes[InputMode.XBoxGamepad].ReadPreferences(JsonConvert.DeserializeObject<Dictionary<string, List<string>>>(((JObject)value).ToString()));
		}
		if (dict.TryGetValue("Mouse And Keyboard UI", out value))
		{
			InputModes[InputMode.KeyboardUI].ReadPreferences(JsonConvert.DeserializeObject<Dictionary<string, List<string>>>(((JObject)value).ToString()));
		}
		if (dict.TryGetValue("Gamepad UI", out value))
		{
			InputModes[InputMode.XBoxGamepadUI].ReadPreferences(JsonConvert.DeserializeObject<Dictionary<string, List<string>>>(((JObject)value).ToString()));
		}
		if (num < 190)
		{
