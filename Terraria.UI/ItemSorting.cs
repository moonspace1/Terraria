...
public class ItemSorting
{
	...
	private class ItemSortingLayers
	{
		...
		{
			List<int> indexesSortable2 = itemsToSort.Where((int i) => inv[i].maxStack == 1 && inv[i].damage > 0 && inv[i].ammo == 0 && inv[i].ranged).ToList();
			layer.Validate(ref indexesSortable2, inv);
			foreach (int item2 in indexesSortable2)
			{
		...
		{
			List<int> indexesSortable3 = itemsToSort.Where((int i) => inv[i].maxStack == 1 && inv[i].damage > 0 && inv[i].ammo == 0 && inv[i].magic).ToList();
			layer.Validate(ref indexesSortable3, inv);
			foreach (int item3 in indexesSortable3)
			{
				itemsToSort.Remove(item3);
			}
			indexesSortable3.Sort(delegate(int x, int y)
			{
				int num3 = inv[y].OriginalRarity.CompareTo(inv[x].OriginalRarity);
				if (num3 == 0)
				{
					num3 = inv[y].OriginalDamage.CompareTo(inv[x].OriginalDamage);
				}
				if (num3 == 0)
				{
					num3 = x.CompareTo(y);
		...
		{
			List<int> indexesSortable4 = itemsToSort.Where((int i) => inv[i].maxStack == 1 && inv[i].damage > 0 && inv[i].summon).ToList();
			layer.Validate(ref indexesSortable4, inv);
			foreach (int item4 in indexesSortable4)
			{
				itemsToSort.Remove(item4);
			}
			indexesSortable4.Sort(delegate(int x, int y)
			{
				int num4 = inv[y].OriginalRarity.CompareTo(inv[x].OriginalRarity);
				if (num4 == 0)
				{
		...
		{
			List<int> indexesSortable5 = itemsToSort.Where((int i) => inv[i].damage > 0 && inv[i].ammo == 0 && inv[i].pick == 0 && inv[i].axe == 0 && inv[i].hammer == 0).ToList();
			layer.Validate(ref indexesSortable5, inv);
			foreach (int item5 in indexesSortable5)
			{
			...
			indexesSortable5.Sort(delegate(int x, int y)
			{
				...
				if (num5 == 0)
				{
					num5 = inv[y].OriginalDamage.CompareTo(inv[x].OriginalDamage);
				}
				if (num5 == 0)
				{
					num5 = x.CompareTo(y);
		...
		{
			List<int> indexesSortable6 = itemsToSort.Where((int i) => inv[i].ammo > 0 && inv[i].damage > 0).ToList();
			layer.Validate(ref indexesSortable6, inv);
			foreach (int item6 in indexesSortable6)
			{
		...
		{
			List<int> indexesSortable7 = itemsToSort.Where((int i) => inv[i].pick > 0 && inv[i].axe > 0).ToList();
			layer.Validate(ref indexesSortable7, inv);
			foreach (int item7 in indexesSortable7)
			{
				itemsToSort.Remove(item7);
			}
			indexesSortable7.Sort((int x, int y) => inv[x].pick.CompareTo(inv[y].pick));
		...
		{
			List<int> indexesSortable8 = itemsToSort.Where((int i) => inv[i].hammer > 0 && inv[i].axe > 0).ToList();
			layer.Validate(ref indexesSortable8, inv);
			foreach (int item8 in indexesSortable8)
			{
				itemsToSort.Remove(item8);
			}
			indexesSortable8.Sort((int x, int y) => inv[x].axe.CompareTo(inv[y].axe));
		...
		{
			List<int> indexesSortable9 = itemsToSort.Where((int i) => inv[i].pick > 0).ToList();
			layer.Validate(ref indexesSortable9, inv);
			foreach (int item9 in indexesSortable9)
			{
				itemsToSort.Remove(item9);
			}
			indexesSortable9.Sort((int x, int y) => inv[x].pick.CompareTo(inv[y].pick));
		...
		{
			List<int> indexesSortable10 = itemsToSort.Where((int i) => inv[i].pick > 0).ToList();
			layer.Validate(ref indexesSortable10, inv);
			foreach (int item10 in indexesSortable10)
			{
				itemsToSort.Remove(item10);
			}
			indexesSortable10.Sort((int x, int y) => inv[x].axe.CompareTo(inv[y].axe));
		...
		{
			List<int> indexesSortable11 = itemsToSort.Where((int i) => inv[i].hammer > 0).ToList();
			layer.Validate(ref indexesSortable11, inv);
			foreach (int item11 in indexesSortable11)
			{
				itemsToSort.Remove(item11);
			}
			indexesSortable11.Sort((int x, int y) => inv[x].hammer.CompareTo(inv[y].hammer));
		...
		{
			List<int> indexesSortable12 = itemsToSort.Where((int i) => inv[i].netID > 0 && ItemID.Sets.SortingPriorityTerraforming[inv[i].netID] > -1).ToList();
			layer.Validate(ref indexesSortable12, inv);
			foreach (int item12 in indexesSortable12)
			{
				itemsToSort.Remove(item12);
			}
			indexesSortable12.Sort(delegate(int x, int y)
			{
				int num7 = ItemID.Sets.SortingPriorityTerraforming[inv[x].netID].CompareTo(ItemID.Sets.SortingPriorityTerraforming[inv[y].netID]);
				if (num7 == 0)
				{
		...
		{
			List<int> indexesSortable13 = itemsToSort.Where((int i) => inv[i].ammo > 0).ToList();
			layer.Validate(ref indexesSortable13, inv);
			foreach (int item13 in indexesSortable13)
			{
		...
		{
			List<int> indexesSortable14 = itemsToSort.Where((int i) => (inv[i].bodySlot >= 0 || inv[i].headSlot >= 0 || inv[i].legSlot >= 0) && !inv[i].vanity).ToList();
			layer.Validate(ref indexesSortable14, inv);
			foreach (int item14 in indexesSortable14)
			{
				itemsToSort.Remove(item14);
			}
			indexesSortable14.Sort(delegate(int x, int y)
			{
				int num9 = inv[y].OriginalRarity.CompareTo(inv[x].OriginalRarity);
				if (num9 == 0)
				{
					num9 = inv[y].OriginalDefense.CompareTo(inv[x].OriginalDefense);
				}
				if (num9 == 0)
				{
					num9 = inv[x].netID.CompareTo(inv[y].netID);
				}
				return num9;
			});
		...
		{
			List<int> indexesSortable15 = itemsToSort.Where((int i) => (inv[i].bodySlot >= 0 || inv[i].headSlot >= 0 || inv[i].legSlot >= 0) && inv[i].vanity).ToList();
			layer.Validate(ref indexesSortable15, inv);
			foreach (int item15 in indexesSortable15)
			{
			...
			indexesSortable15.Sort(delegate(int x, int y)
			{
				...
				if (num10 == 0)
				{
					num10 = inv[x].netID.CompareTo(inv[y].netID);
				}
				return num10;
			});
		...
		{
			List<int> indexesSortable16 = itemsToSort.Where((int i) => inv[i].accessory).ToList();
			layer.Validate(ref indexesSortable16, inv);
			foreach (int item16 in indexesSortable16)
			{
				itemsToSort.Remove(item16);
			}
			indexesSortable16.Sort(delegate(int x, int y)
			{
				int num11 = inv[x].vanity.CompareTo(inv[y].vanity);
				if (num11 == 0)
				{
					num11 = inv[y].OriginalRarity.CompareTo(inv[x].OriginalRarity);
				}
				if (num11 == 0)
				{
					num11 = inv[y].OriginalDefense.CompareTo(inv[x].OriginalDefense);
				}
				if (num11 == 0)
				{
					num11 = inv[x].netID.CompareTo(inv[y].netID);
				}
				return num11;
			});
		...
		{
			List<int> indexesSortable17 = itemsToSort.Where((int i) => Main.projHook[inv[i].shoot]).ToList();
			layer.Validate(ref indexesSortable17, inv);
			foreach (int item17 in indexesSortable17)
			{
		...
		{
			List<int> indexesSortable18 = itemsToSort.Where((int i) => inv[i].mountType != -1 && !MountID.Sets.Cart[inv[i].mountType]).ToList();
			layer.Validate(ref indexesSortable18, inv);
			foreach (int item18 in indexesSortable18)
			{
		...
		{
			List<int> indexesSortable19 = itemsToSort.Where((int i) => inv[i].mountType != -1 && MountID.Sets.Cart[inv[i].mountType]).ToList();
			layer.Validate(ref indexesSortable19, inv);
			foreach (int item19 in indexesSortable19)
			{
		...
		{
			List<int> indexesSortable20 = itemsToSort.Where((int i) => inv[i].buffType > 0 && Main.lightPet[inv[i].buffType]).ToList();
			layer.Validate(ref indexesSortable20, inv);
			foreach (int item20 in indexesSortable20)
			{
				itemsToSort.Remove(item20);
			}
			indexesSortable20.Sort(delegate(int x, int y)
			{
				int num15 = inv[y].OriginalRarity.CompareTo(inv[x].OriginalRarity);
				if (num15 == 0)
				{
		...
		{
			List<int> indexesSortable21 = itemsToSort.Where((int i) => inv[i].buffType > 0 && Main.vanityPet[inv[i].buffType]).ToList();
			layer.Validate(ref indexesSortable21, inv);
			foreach (int item21 in indexesSortable21)
			{
				itemsToSort.Remove(item21);
			}
			indexesSortable21.Sort(delegate(int x, int y)
			{
				int num16 = inv[y].OriginalRarity.CompareTo(inv[x].OriginalRarity);
				if (num16 == 0)
				{
		...
		{
			List<int> indexesSortable22 = itemsToSort.Where((int i) => inv[i].consumable && inv[i].healLife > 0 && inv[i].healMana < 1).ToList();
			layer.Validate(ref indexesSortable22, inv);
			foreach (int item22 in indexesSortable22)
			{
		...
		public static ItemSortingLayer PotionsMana = new ItemSortingLayer("Potions - Mana", delegate(ItemSortingLayer layer, Item[] inv, List<int> itemsToSort)
		{
			...
			foreach (int item23 in indexesSortable23)
			{
				itemsToSort.Remove(item23);
			}
			indexesSortable23.Sort(delegate(int x, int y)
			{
				int num18 = inv[y].healMana.CompareTo(inv[x].healMana);
				if (num18 == 0)
				{
		...
		{
			List<int> indexesSortable24 = itemsToSort.Where((int i) => inv[i].consumable && inv[i].healLife > 0 && inv[i].healMana > 0).ToList();
			layer.Validate(ref indexesSortable24, inv);
			foreach (int item24 in indexesSortable24)
			{
				itemsToSort.Remove(item24);
			}
			indexesSortable24.Sort(delegate(int x, int y)
			{
				int num19 = inv[y].healLife.CompareTo(inv[x].healLife);
				if (num19 == 0)
				{
		...
		{
			List<int> indexesSortable25 = itemsToSort.Where((int i) => inv[i].consumable && inv[i].buffType > 0).ToList();
			layer.Validate(ref indexesSortable25, inv);
			foreach (int item25 in indexesSortable25)
			{
		...
		{
			List<int> indexesSortable26 = itemsToSort.Where((int i) => inv[i].dye > 0).ToList();
			layer.Validate(ref indexesSortable26, inv);
			foreach (int item26 in indexesSortable26)
			{
			...
			indexesSortable26.Sort(delegate(int x, int y)
			{
				...
				if (num21 == 0)
				{
					num21 = inv[y].dye.CompareTo(inv[x].dye);
				}
				if (num21 == 0)
				{
		...
		{
			List<int> indexesSortable27 = itemsToSort.Where((int i) => inv[i].hairDye >= 0).ToList();
			layer.Validate(ref indexesSortable27, inv);
			foreach (int item27 in indexesSortable27)
			{
			...
			indexesSortable27.Sort(delegate(int x, int y)
			{
				...
				if (num22 == 0)
				{
					num22 = inv[y].hairDye.CompareTo(inv[x].hairDye);
				}
				if (num22 == 0)
				{
		...
		{
			List<int> indexesSortable28 = itemsToSort.Where((int i) => inv[i].netID > 0 && ItemID.Sets.SortingPriorityBossSpawns[inv[i].netID] > -1).ToList();
			layer.Validate(ref indexesSortable28, inv);
			foreach (int item28 in indexesSortable28)
			{
				itemsToSort.Remove(item28);
			}
			indexesSortable28.Sort(delegate(int x, int y)
			{
				int num23 = ItemID.Sets.SortingPriorityBossSpawns[inv[x].netID].CompareTo(ItemID.Sets.SortingPriorityBossSpawns[inv[y].netID]);
				if (num23 == 0)
				{
		...
		{
			List<int> indexesSortable29 = itemsToSort.Where((int i) => (inv[i].netID > 0 && ItemID.Sets.SortingPriorityWiring[inv[i].netID] > -1) || inv[i].mech).ToList();
			layer.Validate(ref indexesSortable29, inv);
			foreach (int item29 in indexesSortable29)
			{
				itemsToSort.Remove(item29);
			}
			indexesSortable29.Sort(delegate(int x, int y)
			{
				int num24 = ItemID.Sets.SortingPriorityWiring[inv[y].netID].CompareTo(ItemID.Sets.SortingPriorityWiring[inv[x].netID]);
				if (num24 == 0)
				{
					num24 = inv[y].OriginalRarity.CompareTo(inv[x].OriginalRarity);
				}
				if (num24 == 0)
				{
					num24 = inv[y].netID.CompareTo(inv[x].netID);
				}
				if (num24 == 0)
				{
					num24 = inv[y].stack.CompareTo(inv[x].stack);
				}
				if (num24 == 0)
				{
					num24 = x.CompareTo(y);
		...
		{
			List<int> indexesSortable30 = itemsToSort.Where((int i) => inv[i].netID > 0 && ItemID.Sets.SortingPriorityMaterials[inv[i].netID] > -1).ToList();
			layer.Validate(ref indexesSortable30, inv);
			foreach (int item30 in indexesSortable30)
			{
				itemsToSort.Remove(item30);
			}
			indexesSortable30.Sort(delegate(int x, int y)
			{
				int num25 = ItemID.Sets.SortingPriorityMaterials[inv[y].netID].CompareTo(ItemID.Sets.SortingPriorityMaterials[inv[x].netID]);
				if (num25 == 0)
				{
		...
		{
			List<int> indexesSortable31 = itemsToSort.Where((int i) => inv[i].netID > 0 && ItemID.Sets.SortingPriorityExtractibles[inv[i].netID] > -1).ToList();
			layer.Validate(ref indexesSortable31, inv);
			foreach (int item31 in indexesSortable31)
			{
				itemsToSort.Remove(item31);
			}
			indexesSortable31.Sort(delegate(int x, int y)
			{
				int num26 = ItemID.Sets.SortingPriorityExtractibles[inv[y].netID].CompareTo(ItemID.Sets.SortingPriorityExtractibles[inv[x].netID]);
				if (num26 == 0)
				{
					num26 = inv[y].stack.CompareTo(inv[x].stack);
				}
				if (num26 == 0)
				{
					num26 = x.CompareTo(y);
		...
		{
			List<int> indexesSortable32 = itemsToSort.Where((int i) => (inv[i].netID > 0 && ItemID.Sets.SortingPriorityPainting[inv[i].netID] > -1) || inv[i].paint > 0).ToList();
			layer.Validate(ref indexesSortable32, inv);
			foreach (int item32 in indexesSortable32)
			{
				itemsToSort.Remove(item32);
			}
			indexesSortable32.Sort(delegate(int x, int y)
			{
				int num27 = ItemID.Sets.SortingPriorityPainting[inv[y].netID].CompareTo(ItemID.Sets.SortingPriorityPainting[inv[x].netID]);
				if (num27 == 0)
				{
					num27 = inv[x].paint.CompareTo(inv[y].paint);
				}
				if (num27 == 0)
				{
					num27 = inv[x].paintCoating.CompareTo(inv[y].paintCoating);
				}
				if (num27 == 0)
				{
		...
		{
			List<int> indexesSortable33 = itemsToSort.Where((int i) => inv[i].netID > 0 && ItemID.Sets.SortingPriorityRopes[inv[i].netID] > -1).ToList();
			layer.Validate(ref indexesSortable33, inv);
			foreach (int item33 in indexesSortable33)
			{
				itemsToSort.Remove(item33);
			}
			indexesSortable33.Sort(delegate(int x, int y)
			{
				int num28 = ItemID.Sets.SortingPriorityRopes[inv[y].netID].CompareTo(ItemID.Sets.SortingPriorityRopes[inv[x].netID]);
				if (num28 == 0)
				{
					num28 = inv[y].stack.CompareTo(inv[x].stack);
				}
				if (num28 == 0)
				{
					num28 = x.CompareTo(y);
		...
		{
			List<int> indexesSortable34 = itemsToSort.Where((int i) => inv[i].createTile < 0 && inv[i].createWall < 1).ToList();
			layer.Validate(ref indexesSortable34, inv);
			foreach (int item34 in indexesSortable34)
			{
			...
			indexesSortable34.Sort(delegate(int x, int y)
			{
				...
				if (num29 == 0)
				{
					num29 = inv[y].value.CompareTo(inv[x].value);
				}
				if (num29 == 0)
				{
		...
		{
			List<int> indexesSortable35 = itemsToSort.Where((int i) => inv[i].createTile >= 0 && Main.tileFrameImportant[inv[i].createTile]).ToList();
			layer.Validate(ref indexesSortable35, inv);
			foreach (int item35 in indexesSortable35)
			{
				itemsToSort.Remove(item35);
			}
			indexesSortable35.Sort(delegate(int x, int y)
			{
				int num30 = string.Compare(inv[x].Name, inv[y].Name, StringComparison.OrdinalIgnoreCase);
				if (num30 == 0)
				{
					num30 = inv[y].stack.CompareTo(inv[x].stack);
				}
				if (num30 == 0)
				{
					num30 = x.CompareTo(y);
		...
		{
			List<int> indexesSortable36 = itemsToSort.Where((int i) => inv[i].createWall > 0 || inv[i].createTile >= 0).ToList();
			layer.Validate(ref indexesSortable36, inv);
			foreach (int item36 in indexesSortable36)
			{
				itemsToSort.Remove(item36);
			}
			indexesSortable36.Sort(delegate(int x, int y)
			{
				int num31 = string.Compare(inv[x].Name, inv[y].Name, StringComparison.OrdinalIgnoreCase);
				if (num31 == 0)
				{
					num31 = inv[y].stack.CompareTo(inv[x].stack);
				}
				if (num31 == 0)
				{
					num31 = x.CompareTo(y);
		...
		{
			List<int> indexesSortable37 = itemsToSort.Where((int i) => inv[i].OriginalRarity >= 0).ToList();
			layer.Validate(ref indexesSortable37, inv);
			foreach (int item37 in indexesSortable37)
			{
			...
			indexesSortable37.Sort(delegate(int x, int y)
			{
				...
				if (num32 == 0)
				{
					num32 = string.Compare(inv[x].Name, inv[y].Name, StringComparison.OrdinalIgnoreCase);
				}
				if (num32 == 0)
				{
		...
		{
			List<int> indexesSortable38 = new List<int>(itemsToSort);
			layer.Validate(ref indexesSortable38, inv);
			foreach (int item38 in indexesSortable38)
			{
				itemsToSort.Remove(item38);
			}
			indexesSortable38.Sort(delegate(int x, int y)
			{
				int num33 = inv[y].value.CompareTo(inv[x].value);
				if (num33 == 0)
				{
