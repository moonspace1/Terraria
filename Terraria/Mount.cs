...
public class Mount
{
	...
	private void DoGemMinecartEffect(Player mountedPlayer, int dustType)
	{
		if (Main.rand.Next(10) == 0)
		{
			...
			Dust dust = Dust.NewDustPerfect(pos, dustType, null);
			dust.noGravity = true;
			dust.fadeIn = 0.6f;
			dust.scale = 0.4f;
			dust.velocity *= 0.25f;
	...
	private void DoConfettiMinecartEffect(Player mountedPlayer)
	{
		...
		while (num2 > 0)
		{
			...
			Vector2 pos = mountedPlayer.Center + vector2 + vector;
			pos = mountedPlayer.RotatedRelativePoint(pos);
			Dust dust = Dust.NewDustPerfect(pos, 139 + Main.rand.Next(4), null);
			dust.noGravity = true;
			dust.fadeIn = 0.6f;
	...
	public void Draw(List<DrawData> playerDrawData, int drawType, Player drawPlayer, Vector2 Position, Color drawColor, SpriteEffects playerEffect, float shadow)
	{
		...
		case 50:
			if (drawType == 0)
			{
				...
				DrawData item = new DrawData(TextureAssets.Extra[207].Value, position, value6, drawColor, num9, origin, scale, spriteEffects);
		...
		case 23:
			if (drawType == 0)
			{
				...
				DrawData item = new DrawData(texture2D, vector7.Floor(), value, drawColor, num9, origin, scale, spriteEffects);
				item.shader = currentShader;
				playerDrawData.Add(item);
