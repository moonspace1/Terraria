	...
	private void BlurPass()
	{
		FastParallel.For(0, Width, delegate(int start, int end, object context)
		{
			for (int i = start; i < end; i++)
			{
				...
				BlurLine(IndexOf(i, Height - 1), IndexOf(i, NonVisiblePadding), -1);
			}
		});
		FastParallel.For(0, Height, delegate(int start, int end, object context)
		{
			...
			{
				BlurLine(IndexOf(0, j), IndexOf(Width - 1 - NonVisiblePadding, j), Height);
				BlurLine(IndexOf(Width - 1, j), IndexOf(NonVisiblePadding, j), -Height);
			}
		});
	}
